(ns discord.main
  "Kotodama WASM entrypoint for the Discord clean-room actor (L4) — Clojure port.
  ...
  facts (`discord.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "discord")
(def tier "L4")
...
(def entity-specs
  [{:entity "Channel"   :plural "channels"   :id-prefix "discord_cha"
    :fields [:type :name :guildId :position :topic :nsfw]        :required [:type]
    :coerce {:type :int :position :int}    :refs {:guildId "Guild"}}
   {:entity "Guild"      :plural "guilds"      :id-prefix "discord_gui"
    :fields [:name :ownerId :description :premiumTier :verificationLevel] :required [:name]
    :coerce {:premiumTier :int :verificationLevel :int}          :refs {}}
   {:entity "Message"     :plural "messages"     :id-prefix "discord_mes"
    :fields [:channelId :content :type :pinned :tts :timestamp]        :required [:content]
    :coerce {:type :int :pinned :bool :tts :bool} :refs {:channelId "Channel"}}
   {:entity "User"   :plural "users"   :id-prefix "discord_use"
    :fields [:username :discriminator :globalName :bot]              :required [:username]
    :coerce {:bot :bool}                       :refs {}}
   {:entity "Role"    :plural "roles"    :id-prefix "discord_rol"
    :fields [:name :color :position :hoist :managed :mentionable]  :required [:name]
    :coerce {:color :int :position :int :hoist :bool :managed :bool :mentionable :bool}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"discord.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "discord-compat" :tier tier :entities entities} 200])
...
(defn start! [] :discord-compat/ready)
